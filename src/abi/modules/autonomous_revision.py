    ...
    *,
    ...
    if client_name == AUTONOMOUS_REVISION_CLIENT_OPENAI:
        return _refusal(
            ...
            reader_lab_packet=reader_lab_packet,
            message=(
                "Autonomous revision OpenAI path is interface-only in v1; "
                "use --client fake for deterministic local repair evidence."
            ),
        )
    ...
    if not reader_lab_packet_dir.exists() or not reader_lab_packet_dir.is_dir():
        return _refusal(
            client_name=client_name,
            model=AUTONOMOUS_REVISION_CLIENT_FAKE,
...
def _build_subject_manifest(subject: RevisionSubject) -> dict[str, Any]:
    return {
        "worker": "autonomous_revision_subject_manifest_v1_fake",
        "run_id": subject.run_id,
...
def _build_closed_loop_gate_report(
    *,
    ...
    unresolved = [
        f"selected failure remains under fake evidence: "
        f"{payloads['selected_failure_diagnosis']['selected_failure_type']}",
        "closed-loop comparison is deterministic fixture evidence",
    ]
    ...
    return {
        "worker": "autonomous_closed_loop_gate_report_v1_fake",
        ...
        "no_phase_shift_claim": True,
        "summary_verdict": (
            "Closed-loop revision created internal repair evidence, but fake fixture mode, "
            "unresolved blockers, and missing operator approval keep autonomous "
...
def _build_packet_summary(
    *,
    ...
    gate_report = payloads["autonomous_closed_loop_gate_report"]
    return {
        "worker": "autonomous_closed_loop_packet_v1_fake",
        "run_id": subject.run_id,
