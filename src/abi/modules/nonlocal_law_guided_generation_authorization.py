"""Controller-owned authorization for one nonlocal law-guided generation attempt."""

from __future__ import annotations

from dataclasses import dataclass
from pathlib import Path
import sqlite3
from typing import Any

from abi.artifacts import ArtifactRecord, list_artifacts, row_to_artifact
from abi.config import AbiConfig
from abi.controller.gates import GateRecord, record_gate
from abi.controller.state import (
    AUTONOMOUS_NONLOCAL_LAW_GUIDED_GENERATION_AUTHORIZATION_ACTIVE_PHASE,
    get_run,
    set_active_phase,
)
from abi.db import connect, initialize_database
from abi.modules.local_law_discovery import DISCOVERED_LOCAL_LAW_ID
from abi.modules.nonlocal_law_guided_strategy import SELECTED_NONLOCAL_STRATEGY_CLASS
from abi.modules.nonlocal_law_guided_work_order import (
    ABLATION_CONTROLS,
    EXPLANATION_POLICY,
    FUTURE_SCHEMA_NAME,
    GENERATION_AUTHORIZATION_ALLOWED_DECISIONS,
    GENERATION_CONTRACT_VERSION,
    MATERIALITY_POLICY_ID,
    NONLOCAL_LAW_GUIDED_WORK_ORDER_KIND,
    NONLOCAL_LAW_TARGET_SCOPE,
    NONLOCAL_TARGET_UNIT_IDS,
    PROMPT_CONTRACT_ID,
    READER_STATE_FOCUS,
    RECOMPOSITION_PRINCIPLE,
    SEMANTIC_VALIDATOR_ID,
)
from abi.modules.post_local_residual_strategy_synthesis import (
    EXPECTED_CURRENT_BEST_PACKET_ID,
    EXPECTED_PROOF_PACKET_ID,
    EXPECTED_READER_STATE_PACKET_ID,
)
from abi.packets import (
    PacketWriter,
    create_packet_dir,
    packet_artifact_count_summary,
    read_json_file,
)


NONLOCAL_LAW_GENERATION_AUTHORIZATION_LINEAGE_ID = (
    "nonlocal_law_guided_generation_authorization_v1"
)
NONLOCAL_LAW_GENERATION_AUTHORIZATION_CREATED_BY = (
    "nonlocal_law_guided_generation_authorization_v1_controller"
)

AUTHORIZATION_DECISION_AUTHORIZE_ONE = (
    "authorize_one_bounded_nonlocal_law_guided_generation"
)
AUTHORIZATION_DECISION_REQUIRE_REVISION = "require_work_order_revision"
AUTHORIZATION_DECISION_PAUSE = "pause_generation"
NONLOCAL_LAW_GENERATION_AUTHORIZATION_DECISIONS = (
    AUTHORIZATION_DECISION_AUTHORIZE_ONE,
    AUTHORIZATION_DECISION_REQUIRE_REVISION,
    AUTHORIZATION_DECISION_PAUSE,
)
NEXT_ACTION_AUTHORIZE = (
    "review_nonlocal_law_generation_authorization_before_candidate_generation"
)
NEXT_ACTION_REQUIRE_REVISION = (
    "revise_nonlocal_law_work_order_before_generation_authorization"
)
NEXT_ACTION_PAUSE = "pause_nonlocal_law_guided_generation"
NEXT_ACTION_GENERATOR_NOT_IMPLEMENTED = (
    "implement_nonlocal_law_guided_candidate_generation"
)

NONLOCAL_LAW_GENERATION_AUTHORIZATION_ARTIFACT_TYPES = (
    "source_work_order_intake_summary",
    "generation_contract_intake_summary",
    "authorization_decision_record",
    "target_unit_authorization_scope",
    "protected_strengths_and_forbidden_imitation_review",
    "materiality_semantic_validation_readiness_report",
    "post_generation_evidence_plan",
    "model_call_budget_report",
    "generation_lock_transition_report",
    "project_health_scope_guard_report",
    "nonlocal_law_generation_authorization_gate_report",
    "nonlocal_law_guided_generation_authorization_packet",
)

REQUIRED_WORK_ORDER_ARTIFACTS = (
    "nonlocal_law_guided_work_order_packet",
    "selected_nonlocal_intervention_scope",
    "law_guided_pressure_recomposition_map",
    "protected_current_best_strengths",
    "forbidden_rival_imitation_inventory",
    "nonlocal_target_unit_map",
    "future_generation_contract",
    "materiality_and_semantic_validation_plan",
    "ablation_and_reader_eval_plan",
    "nonlocal_law_work_order_gate_report",
    "project_health_scope_guard_report",
)


@dataclass(frozen=True)
class NonlocalLawGenerationAuthorizationResult:
    exit_code: int
    payload: dict[str, object]
    artifacts: tuple[ArtifactRecord, ...] = ()
    gate_record: GateRecord | None = None


@dataclass(frozen=True)
class NonlocalLawGenerationAuthorizationSubject:
    run_id: str
    work_order_packet_dir: Path
    work_order_packet_id: str
    work_order_packet_artifact_id: str | None
    work_order_artifact_ids: dict[str, str]
    payloads: dict[str, dict[str, Any]]
    source_parent_ids: tuple[str, ...]


def run_nonlocal_law_generation_authorization(
    config: AbiConfig,
    *,
    work_order_packet: Path | str,
    operator_reviewed: bool,
    decision: str | None,
) -> NonlocalLawGenerationAuthorizationResult:
    initialize_database(config)
    work_order_packet_dir = _resolve_path(config, work_order_packet)
    if not operator_reviewed:
        return _refusal(
            work_order_packet=work_order_packet_dir,
            decision=decision,
            message=(
                "Nonlocal law-guided generation authorization refused; "
                "--operator-reviewed is required."
            ),
        )
    if decision not in NONLOCAL_LAW_GENERATION_AUTHORIZATION_DECISIONS:
        return _refusal(
            work_order_packet=work_order_packet_dir,
            decision=decision,
            message=(
                "Nonlocal law-guided generation authorization refused; "
                "--decision must be one of: "
                f"{', '.join(NONLOCAL_LAW_GENERATION_AUTHORIZATION_DECISIONS)}."
            ),
        )
    if not work_order_packet_dir.exists() or not work_order_packet_dir.is_dir():
        return _refusal(
            work_order_packet=work_order_packet_dir,
            decision=decision,
            message=(
                "Nonlocal law-guided generation authorization refused; "
                f"work-order packet directory not found: {work_order_packet_dir}"
            ),
        )

    try:
        subject = _load_subject(config, work_order_packet_dir)
    except ValueError as error:
        return _refusal(
            work_order_packet=work_order_packet_dir,
            decision=decision,
            message=str(error),
        )

    with connect(config.db_path) as connection:
        if get_run(connection, subject.run_id) is None:
            return _refusal(
                work_order_packet=work_order_packet_dir,
                decision=decision,
                message=(
                    "Nonlocal law-guided generation authorization refused; "
                    f"run is not registered: {subject.run_id}"
                ),
            )
        duplicate = _unconsumed_authorization_for_work_order(connection, subject)
        if duplicate is not None:
            return _refusal(
                work_order_packet=work_order_packet_dir,
                decision=decision,
                message=(
                    "Nonlocal law-guided generation authorization refused; "
                    "an unconsumed current-valid authorization already exists "
                    f"for this work order: {duplicate.id}"
                ),
            )

        set_active_phase(
            connection,
            subject.run_id,
            AUTONOMOUS_NONLOCAL_LAW_GUIDED_GENERATION_AUTHORIZATION_ACTIVE_PHASE,
        )
        packet_dir = create_packet_dir(
            config.run_dir(subject.run_id)
            / "nonlocal_law_guided_generation_authorization"
        )
        writer = PacketWriter(
            connection=connection,
            run_id=subject.run_id,
            packet_dir=packet_dir,
            lineage_id=NONLOCAL_LAW_GENERATION_AUTHORIZATION_LINEAGE_ID,
            created_by=NONLOCAL_LAW_GENERATION_AUTHORIZATION_CREATED_BY,
            fixture_only=False,
            model_call_id=None,
        )

        payloads: dict[str, dict[str, object]] = {}
        artifacts: dict[str, ArtifactRecord] = {}

        payloads["source_work_order_intake_summary"] = _build_source_intake(
            subject,
            packet_dir,
        )
        artifacts["source_work_order_intake_summary"] = writer.write_artifact(
            "source_work_order_intake_summary",
            payloads["source_work_order_intake_summary"],
            parent_ids=list(subject.source_parent_ids),
        )

        payloads["generation_contract_intake_summary"] = _build_contract_intake(
            subject,
            decision,
        )
        artifacts["generation_contract_intake_summary"] = writer.write_artifact(
            "generation_contract_intake_summary",
            payloads["generation_contract_intake_summary"],
            parent_ids=[artifacts["source_work_order_intake_summary"].id],
        )

        payloads["authorization_decision_record"] = _build_decision_record(
            subject,
            decision,
        )
        artifacts["authorization_decision_record"] = writer.write_artifact(
            "authorization_decision_record",
            payloads["authorization_decision_record"],
            parent_ids=[artifacts["generation_contract_intake_summary"].id],
        )

        payloads["target_unit_authorization_scope"] = _build_target_scope(
            subject,
            decision,
        )
        artifacts["target_unit_authorization_scope"] = writer.write_artifact(
            "target_unit_authorization_scope",
            payloads["target_unit_authorization_scope"],
            parent_ids=[artifacts["authorization_decision_record"].id],
        )

        payloads["protected_strengths_and_forbidden_imitation_review"] = (
            _build_protected_and_forbidden_review(subject, decision)
        )
        artifacts["protected_strengths_and_forbidden_imitation_review"] = (
            writer.write_artifact(
                "protected_strengths_and_forbidden_imitation_review",
                payloads["protected_strengths_and_forbidden_imitation_review"],
                parent_ids=[artifacts["target_unit_authorization_scope"].id],
            )
        )

        payloads["materiality_semantic_validation_readiness_report"] = (
            _build_validation_readiness(subject, decision)
        )
        artifacts["materiality_semantic_validation_readiness_report"] = (
            writer.write_artifact(
                "materiality_semantic_validation_readiness_report",
                payloads["materiality_semantic_validation_readiness_report"],
                parent_ids=[
                    artifacts[
                        "protected_strengths_and_forbidden_imitation_review"
                    ].id
                ],
            )
        )

        payloads["post_generation_evidence_plan"] = _build_evidence_plan(
            subject,
            decision,
        )
        artifacts["post_generation_evidence_plan"] = writer.write_artifact(
            "post_generation_evidence_plan",
            payloads["post_generation_evidence_plan"],
            parent_ids=[
                artifacts["materiality_semantic_validation_readiness_report"].id
            ],
        )

        payloads["model_call_budget_report"] = _build_model_budget(
            subject,
            decision,
        )
        artifacts["model_call_budget_report"] = writer.write_artifact(
            "model_call_budget_report",
            payloads["model_call_budget_report"],
            parent_ids=[artifacts["post_generation_evidence_plan"].id],
        )

        payloads["generation_lock_transition_report"] = _build_lock_transition(
            subject,
            decision,
        )
        artifacts["generation_lock_transition_report"] = writer.write_artifact(
            "generation_lock_transition_report",
            payloads["generation_lock_transition_report"],
            parent_ids=[artifacts["model_call_budget_report"].id],
        )

        payloads["project_health_scope_guard_report"] = _build_health_report(
            subject,
            decision,
            payloads,
        )
        artifacts["project_health_scope_guard_report"] = writer.write_artifact(
            "project_health_scope_guard_report",
            payloads["project_health_scope_guard_report"],
            parent_ids=[artifacts["generation_lock_transition_report"].id],
        )

        payloads["nonlocal_law_generation_authorization_gate_report"] = (
            _build_gate_report(subject, decision, payloads)
        )
        artifacts["nonlocal_law_generation_authorization_gate_report"] = (
            writer.write_artifact(
                "nonlocal_law_generation_authorization_gate_report",
                payloads["nonlocal_law_generation_authorization_gate_report"],
                parent_ids=[artifacts["project_health_scope_guard_report"].id],
            )
        )

        payloads["nonlocal_law_guided_generation_authorization_packet"] = (
            _build_packet_summary(
                subject=subject,
                packet_dir=packet_dir,
                decision=decision,
                payloads=payloads,
                artifacts=artifacts,
            )
        )
        artifacts["nonlocal_law_guided_generation_authorization_packet"] = (
            writer.write_artifact(
                "nonlocal_law_guided_generation_authorization_packet",
                payloads["nonlocal_law_guided_generation_authorization_packet"],
                parent_ids=[
                    artifact.id
                    for artifact_type, artifact in artifacts.items()
                    if artifact_type
                    != "nonlocal_law_guided_generation_authorization_packet"
                ],
            )
        )

        gate_report = payloads["nonlocal_law_generation_authorization_gate_report"]
        gate_record = record_gate(
            connection,
            run_id=subject.run_id,
            gate_name="nonlocal_law_generation_authorization_gate_report",
            passed=False,
            blocking_defects=list(gate_report["unresolved_blockers"]),
            lineage_id=NONLOCAL_LAW_GENERATION_AUTHORIZATION_LINEAGE_ID,
        )

    return NonlocalLawGenerationAuthorizationResult(
        exit_code=0,
        payload=_result_payload(
            packet_dir=packet_dir,
            artifacts=artifacts,
            payloads=payloads,
        ),
        artifacts=tuple(artifacts.values()),
        gate_record=gate_record,
    )


def run_nonlocal_law_candidate_generation_placeholder(
    config: AbiConfig,
    *,
    client_name: str,
    authorization_packet: Path | str,
    allow_live_model: bool,
) -> NonlocalLawGenerationAuthorizationResult:
    initialize_database(config)
    resolved_packet = _resolve_path(config, authorization_packet)
    payload = _placeholder_generation_payload(
        authorization_packet=resolved_packet,
        client_name=client_name,
        message=(
            "Nonlocal law-guided candidate generation refused; "
            "--allow-live-model is required."
        )
        if not allow_live_model
        else (
            "Nonlocal law-guided candidate generation refused; full generation "
            "worker is not implemented in this task."
        ),
    )
    return NonlocalLawGenerationAuthorizationResult(exit_code=1, payload=payload)


def _load_subject(
    config: AbiConfig,
    work_order_packet_dir: Path,
) -> NonlocalLawGenerationAuthorizationSubject:
    payloads = _load_required_payloads(work_order_packet_dir)
    packet = payloads["nonlocal_law_guided_work_order_packet"]
    run_id = _required_string(
        packet.get("run_id"),
        "Nonlocal law-guided generation authorization refused; "
        "work-order packet missing run_id.",
    )
    work_order_packet_id = str(packet.get("packet_id") or work_order_packet_dir.name)
    superseding_packet = _newer_corrected_work_order_packet_id(
        config=config,
        run_id=run_id,
        work_order_packet_dir=work_order_packet_dir,
        source_strategy_packet_id=str(packet.get("source_strategy_packet_id") or ""),
    )
    if superseding_packet:
        raise ValueError(
            "Nonlocal law-guided generation authorization refused; work-order "
            f"packet was superseded by newer corrected packet: {superseding_packet}."
        )
    _validate_work_order_payloads(payloads)

    packet_path = (
        work_order_packet_dir / "nonlocal_law_guided_work_order_packet.json"
    )
    with connect(config.db_path) as connection:
        work_order_artifact = _artifact_for_path(connection, packet_path)
    work_order_artifact_ids = _artifact_ids_from_packet(packet)
    source_parent_ids = _unique(
        [
            work_order_artifact.id if work_order_artifact else None,
            *work_order_artifact_ids.values(),
        ]
    )
    return NonlocalLawGenerationAuthorizationSubject(
        run_id=run_id,
        work_order_packet_dir=work_order_packet_dir,
        work_order_packet_id=work_order_packet_id,
        work_order_packet_artifact_id=work_order_artifact.id
        if work_order_artifact
        else None,
        work_order_artifact_ids=work_order_artifact_ids,
        payloads=payloads,
        source_parent_ids=tuple(source_parent_ids),
    )


def _load_required_payloads(packet_dir: Path) -> dict[str, dict[str, Any]]:
    payloads: dict[str, dict[str, Any]] = {}
    for artifact_type in REQUIRED_WORK_ORDER_ARTIFACTS:
        path = packet_dir / f"{artifact_type}.json"
        if not path.exists():
            raise ValueError(
                "Nonlocal law-guided generation authorization refused; "
                f"work-order packet missing {path.name}."
            )
        payloads[artifact_type] = _read_envelope_payload(path)
    return payloads


def _read_envelope_payload(path: Path) -> dict[str, Any]:
    envelope = read_json_file(path)
    if not isinstance(envelope, dict) or not isinstance(envelope.get("payload"), dict):
        raise ValueError(
            "Nonlocal law-guided generation authorization refused; malformed "
            f"work-order artifact: {path.name}."
        )
    return envelope["payload"]


def _validate_work_order_payloads(payloads: dict[str, dict[str, Any]]) -> None:
    packet = payloads["nonlocal_law_guided_work_order_packet"]
    scope = payloads["selected_nonlocal_intervention_scope"]
    recomposition = payloads["law_guided_pressure_recomposition_map"]
    forbidden = payloads["forbidden_rival_imitation_inventory"]
    units = payloads["nonlocal_target_unit_map"]
    contract = payloads["future_generation_contract"]
    validation = payloads["materiality_and_semantic_validation_plan"]
    evidence = payloads["ablation_and_reader_eval_plan"]

    _require_bool(packet, "accepted", True)
    _require_equal(packet, "work_order_kind", NONLOCAL_LAW_GUIDED_WORK_ORDER_KIND)
    _require_equal(packet, "target_scope", NONLOCAL_LAW_TARGET_SCOPE)
    _require_equal(packet, "law_id", DISCOVERED_LOCAL_LAW_ID)
    _require_equal(
        packet,
        "current_best_candidate_packet_id",
        EXPECTED_CURRENT_BEST_PACKET_ID,
    )
    _require_equal(packet, "proof_packet_id", EXPECTED_PROOF_PACKET_ID)
    _require_equal(packet, "reader_state_packet_id", EXPECTED_READER_STATE_PACKET_ID)
    _require_equal(packet, "selected_strategy_class", SELECTED_NONLOCAL_STRATEGY_CLASS)
    _require_bool(packet, "ready_for_generation_authorization_review", True)
    _require_bool(contract, "generation_authorization_decision_required", True)
    _require_bool(contract, "future_generation_requires_separate_authorization", True)
    _require_bool(contract, "future_generation_authorized", False)
    _require_equal(contract, "generation_attempt_budget", 0)
    _require_equal(packet, "generation_attempt_budget", 0)
    _require_bool(packet, "candidate_generated", False)
    _require_bool(packet, "generation_authorized", False)
    _require_bool(packet, "future_generation_authorized", False)

    _require_equal(contract, "generation_contract_version", GENERATION_CONTRACT_VERSION)
    _require_equal(contract, "prompt_contract_id", PROMPT_CONTRACT_ID)
    _require_equal(contract, "materiality_policy_id", MATERIALITY_POLICY_ID)
    _require_equal(contract, "semantic_validator_id", SEMANTIC_VALIDATOR_ID)
    _require_equal(contract, "schema", FUTURE_SCHEMA_NAME)

    _require_equal(scope, "intervention_scope", NONLOCAL_LAW_TARGET_SCOPE)
    _require_bool(scope, "nonlocal_but_bounded", True)
    _require_bool(scope, "free_rewrite_allowed", False)
    _require_bool(scope, "one_region_patch_allowed", False)
    _require_bool(scope, "generation_allowed", False)
    _require_equal(recomposition, "recomposition_principle", RECOMPOSITION_PRINCIPLE)
    _require_equal(recomposition, "explanation_policy", EXPLANATION_POLICY)
    _require_bool(recomposition, "nonlocal_not_local_patch", True)

    target_unit_ids = _string_list(units.get("target_unit_ids"))
    missing_units = [
        unit_id for unit_id in NONLOCAL_TARGET_UNIT_IDS if unit_id not in target_unit_ids
    ]
    if missing_units:
        raise ValueError(
            "Nonlocal law-guided generation authorization refused; target units "
            f"missing required units: {', '.join(missing_units)}."
        )
    if not _string_list(validation.get("materiality_requirements")):
        raise ValueError(
            "Nonlocal law-guided generation authorization refused; materiality "
            "requirements are missing."
        )
    if not _string_list(validation.get("semantic_validation_requirements")):
        raise ValueError(
            "Nonlocal law-guided generation authorization refused; semantic "
            "validation requirements are missing."
        )
    if not _string_list(forbidden.get("forbidden_rival_objects_or_sequence")):
        raise ValueError(
            "Nonlocal law-guided generation authorization refused; forbidden "
            "rival imitation inventory is missing."
        )
    _require_bool(forbidden, "rival_imitation_forbidden", True)
    _require_bool(forbidden, "non_imitation_constraints_passed", True)
    if not set(ABLATION_CONTROLS) <= set(_string_list(evidence.get("ablation_controls"))):
        raise ValueError(
            "Nonlocal law-guided generation authorization refused; ablation "
            "controls are missing."
        )
    if not set(READER_STATE_FOCUS) <= set(_string_list(evidence.get("reader_state_focus"))):
        raise ValueError(
            "Nonlocal law-guided generation authorization refused; reader-state "
            "focus is missing."
        )
    if _has_final_or_phase_claim(payloads):
        raise ValueError(
            "Nonlocal law-guided generation authorization refused; finality, "
            "phase-shift, or strongest-rival defeat claim appears in the work "
            "order packet."
        )


def _build_source_intake(
    subject: NonlocalLawGenerationAuthorizationSubject,
    packet_dir: Path,
) -> dict[str, object]:
    packet = subject.payloads["nonlocal_law_guided_work_order_packet"]
    return {
        "run_id": subject.run_id,
        "packet_id": packet_dir.name,
        "packet_dir": str(packet_dir),
        "source_work_order_packet_id": subject.work_order_packet_id,
        "source_work_order_packet_dir": str(subject.work_order_packet_dir),
        "source_work_order_packet_artifact_id": subject.work_order_packet_artifact_id,
        "source_strategy_packet_id": packet.get("source_strategy_packet_id"),
        "source_diagnostic_packet_id": packet.get("source_diagnostic_packet_id"),
        "current_best_candidate_packet_id": packet.get("current_best_candidate_packet_id"),
        "proof_packet_id": packet.get("proof_packet_id"),
        "reader_state_packet_id": packet.get("reader_state_packet_id"),
        "law_id": packet.get("law_id"),
        "selected_strategy_class": packet.get("selected_strategy_class"),
        "work_order_kind": packet.get("work_order_kind"),
        "target_scope": packet.get("target_scope"),
        "source_work_order_current_valid": True,
        "generation_authorized": False,
        "next_generation_authorized": False,
        "authorization_consumed": False,
        "candidate_generated": False,
        "model_calls": 0,
        "finalization_eligible": False,
        "no_final_claim": True,
        "no_phase_shift_claim": True,
        "worker": "source_work_order_intake_summary_v1_controller",
    }


def _build_contract_intake(
    subject: NonlocalLawGenerationAuthorizationSubject,
    decision: str,
) -> dict[str, object]:
    contract = subject.payloads["future_generation_contract"]
    generation_authorized = decision == AUTHORIZATION_DECISION_AUTHORIZE_ONE
    return {
        "generation_contract_version": contract["generation_contract_version"],
        "prompt_contract_id": contract["prompt_contract_id"],
        "materiality_policy_id": contract["materiality_policy_id"],
        "semantic_validator_id": contract["semantic_validator_id"],
        "schema": contract["schema"],
        "source_future_generation_authorized": False,
        "future_generation_requires_separate_authorization": True,
        "generation_authorization_decision_required": True,
        "decision": decision,
        "generation_authorized": generation_authorized,
        "next_generation_authorized": generation_authorized,
        "generation_attempt_budget": 1 if generation_authorized else 0,
        "authorization_consumed": False,
        "candidate_generated": False,
        "model_calls": 0,
        "finalization_eligible": False,
        "no_final_claim": True,
        "no_phase_shift_claim": True,
        "worker": "generation_contract_intake_summary_v1_controller",
    }


def _build_decision_record(
    subject: NonlocalLawGenerationAuthorizationSubject,
    decision: str,
) -> dict[str, object]:
    generation_authorized = decision == AUTHORIZATION_DECISION_AUTHORIZE_ONE
    return {
        "source_work_order_packet_id": subject.work_order_packet_id,
        "operator_reviewed": True,
        "decision": decision,
        "allowed_decisions": list(GENERATION_AUTHORIZATION_ALLOWED_DECISIONS),
        "decision_allowed": True,
        "generation_authorized": generation_authorized,
        "next_generation_authorized": generation_authorized,
        "generation_attempt_budget": 1 if generation_authorized else 0,
        "authorization_consumed": False,
        "candidate_generated": False,
        "model_calls": 0,
        "next_recommended_action": _next_action(decision),
        "finalization_eligible": False,
        "no_final_claim": True,
        "no_phase_shift_claim": True,
        "worker": "authorization_decision_record_v1_controller",
    }


def _build_target_scope(
    subject: NonlocalLawGenerationAuthorizationSubject,
    decision: str,
) -> dict[str, object]:
    packet = subject.payloads["nonlocal_law_guided_work_order_packet"]
    scope = subject.payloads["selected_nonlocal_intervention_scope"]
    units = subject.payloads["nonlocal_target_unit_map"]
    generation_authorized = decision == AUTHORIZATION_DECISION_AUTHORIZE_ONE
    return {
        "source_work_order_packet_id": subject.work_order_packet_id,
        "target_scope": packet["target_scope"],
        "intervention_scope": packet["intervention_scope"],
        "affected_regions": list(packet["affected_regions"]),
        "protected_regions": list(packet["protected_regions"]),
        "nonlocal_but_bounded": scope["nonlocal_but_bounded"],
        "free_rewrite_allowed": False,
        "one_region_patch_allowed": False,
        "target_unit_ids": list(units["target_unit_ids"]),
        "target_units": list(units["target_units"]),
        "target_unit_count": len(units["target_unit_ids"]),
        "generation_authorized": generation_authorized,
        "next_generation_authorized": generation_authorized,
        "generation_attempt_budget": 1 if generation_authorized else 0,
        "authorization_consumed": False,
        "candidate_generated": False,
        "model_calls": 0,
        "no_final_claim": True,
        "no_phase_shift_claim": True,
        "worker": "target_unit_authorization_scope_v1_controller",
    }


def _build_protected_and_forbidden_review(
    subject: NonlocalLawGenerationAuthorizationSubject,
    decision: str,
) -> dict[str, object]:
    protected = subject.payloads["protected_current_best_strengths"]
    forbidden = subject.payloads["forbidden_rival_imitation_inventory"]
    generation_authorized = decision == AUTHORIZATION_DECISION_AUTHORIZE_ONE
    return {
        "source_work_order_packet_id": subject.work_order_packet_id,
        "protected_strengths": list(protected["protected_strengths"]),
        "must_preserve": list(protected["must_preserve"]),
        "forbidden_rival_objects_or_sequence": list(
            forbidden["forbidden_rival_objects_or_sequence"]
        ),
        "forbidden_rival_imitation_modes": list(
            forbidden["forbidden_rival_imitation_modes"]
        ),
        "forbidden_regressions": list(forbidden["forbidden_regressions"]),
        "rival_imitation_forbidden": True,
        "non_imitation_constraints_passed": True,
        "strongest_rival_defeated": False,
        "strongest_rival_pressure_remains_blocking": True,
        "generation_authorized": generation_authorized,
        "next_generation_authorized": generation_authorized,
        "authorization_consumed": False,
        "candidate_generated": False,
        "model_calls": 0,
        "no_final_claim": True,
        "no_phase_shift_claim": True,
        "worker": "protected_strengths_and_forbidden_imitation_review_v1_controller",
    }


def _build_validation_readiness(
    subject: NonlocalLawGenerationAuthorizationSubject,
    decision: str,
) -> dict[str, object]:
    validation = subject.payloads["materiality_and_semantic_validation_plan"]
    generation_authorized = decision == AUTHORIZATION_DECISION_AUTHORIZE_ONE
    return {
        "source_work_order_packet_id": subject.work_order_packet_id,
        "materiality_policy_id": validation["materiality_policy_id"],
        "semantic_validator_id": validation["semantic_validator_id"],
        "materiality_requirements": list(validation["materiality_requirements"]),
        "semantic_validation_requirements": list(
            validation["semantic_validation_requirements"]
        ),
        "target_unit_ids_covered": list(validation["target_unit_ids_covered"]),
        "protected_unit_ids_covered": list(validation["protected_unit_ids_covered"]),
        "validation_required_before_candidate_evidence": True,
        "generation_authorized": generation_authorized,
        "next_generation_authorized": generation_authorized,
        "authorization_consumed": False,
        "candidate_generated": False,
        "model_calls": 0,
        "no_final_claim": True,
        "no_phase_shift_claim": True,
        "worker": "materiality_semantic_validation_readiness_report_v1_controller",
    }


def _build_evidence_plan(
    subject: NonlocalLawGenerationAuthorizationSubject,
    decision: str,
) -> dict[str, object]:
    evidence = subject.payloads["ablation_and_reader_eval_plan"]
    generation_authorized = decision == AUTHORIZATION_DECISION_AUTHORIZE_ONE
    return {
        "source_work_order_packet_id": subject.work_order_packet_id,
        "ablation_controls": list(evidence["ablation_controls"]),
        "reader_state_focus": list(evidence["reader_state_focus"]),
        "post_generation_ablation_required": True,
        "post_generation_reader_state_eval_required": True,
        "post_generation_synthesis_required": True,
        "strongest_rival_pressure_remains_active_until_synthesis": True,
        "ablation_executed": False,
        "reader_state_eval_executed": False,
        "generation_authorized": generation_authorized,
        "next_generation_authorized": generation_authorized,
        "authorization_consumed": False,
        "candidate_generated": False,
        "model_calls": 0,
        "no_final_claim": True,
        "no_phase_shift_claim": True,
        "worker": "post_generation_evidence_plan_v1_controller",
    }


def _build_model_budget(
    subject: NonlocalLawGenerationAuthorizationSubject,
    decision: str,
) -> dict[str, object]:
    generation_authorized = decision == AUTHORIZATION_DECISION_AUTHORIZE_ONE
    return {
        "source_work_order_packet_id": subject.work_order_packet_id,
        "decision": decision,
        "generation_authorized": generation_authorized,
        "next_generation_authorized": generation_authorized,
        "generation_attempt_budget": 1 if generation_authorized else 0,
        "max_model_calls_for_future_generation": 1 if generation_authorized else 0,
        "model_calls_made_now": 0,
        "model_calls": 0,
        "authorization_consumed": False,
        "candidate_generated": False,
        "no_final_claim": True,
        "no_phase_shift_claim": True,
        "worker": "model_call_budget_report_v1_controller",
    }


def _build_lock_transition(
    subject: NonlocalLawGenerationAuthorizationSubject,
    decision: str,
) -> dict[str, object]:
    generation_authorized = decision == AUTHORIZATION_DECISION_AUTHORIZE_ONE
    return {
        "source_work_order_packet_id": subject.work_order_packet_id,
        "source_work_order_future_generation_authorized": False,
        "decision": decision,
        "generation_authorized": generation_authorized,
        "next_generation_authorized": generation_authorized,
        "generation_attempt_budget": 1 if generation_authorized else 0,
        "authorization_consumed": False,
        "candidate_generated": False,
        "model_calls": 0,
        "transition_summary": (
            "One bounded future nonlocal law-guided generation attempt is "
            "authorized but unconsumed."
        )
        if generation_authorized
        else "No generation attempt is authorized by this decision.",
        "next_recommended_action": _next_action(decision),
        "no_final_claim": True,
        "no_phase_shift_claim": True,
        "worker": "generation_lock_transition_report_v1_controller",
    }


def _build_health_report(
    subject: NonlocalLawGenerationAuthorizationSubject,
    decision: str,
    payloads: dict[str, dict[str, object]],
) -> dict[str, object]:
    packet = subject.payloads["nonlocal_law_guided_work_order_packet"]
    generation_authorized = decision == AUTHORIZATION_DECISION_AUTHORIZE_ONE
    checks = [
        _check("source_work_order_accepted", packet.get("accepted") is True),
        _check("source_work_order_current_valid", True),
        _check("decision_allowed", decision in NONLOCAL_LAW_GENERATION_AUTHORIZATION_DECISIONS),
        _check("generation_attempt_budget_bounded", True),
        _check("authorization_unconsumed", True),
        _check("no_candidate_generated", True),
        _check("no_model_calls", True),
        _check("no_final_claim", True),
        _check("no_phase_shift_claim", True),
    ]
    passed = all(bool(check["passed"]) for check in checks)
    return {
        "checks": checks,
        "passed": passed,
        "project_health_scope_guard_passed": passed,
        "source_chain_coherent": True,
        "decision": decision,
        "generation_authorized": generation_authorized,
        "next_generation_authorized": generation_authorized,
        "generation_attempt_budget": 1 if generation_authorized else 0,
        "authorization_consumed": False,
        "candidate_generated": False,
        "model_calls": 0,
        "gate_report_available": "nonlocal_law_generation_authorization_gate_report"
        in payloads,
        "finalization_eligible": False,
        "no_final_claim": True,
        "no_phase_shift_claim": True,
        "worker": "project_health_scope_guard_report_v1_controller",
    }


def _build_gate_report(
    subject: NonlocalLawGenerationAuthorizationSubject,
    decision: str,
    payloads: dict[str, dict[str, object]],
) -> dict[str, object]:
    generation_authorized = decision == AUTHORIZATION_DECISION_AUTHORIZE_ONE
    health = payloads["project_health_scope_guard_report"]
    gate_results = [
        _gate_result("source_work_order_consumed", True),
        _gate_result("operator_review_recorded", True),
        _gate_result("authorization_decision_recorded", True),
        _gate_result("target_units_preserved", True),
        _gate_result("materiality_requirements_preserved", True),
        _gate_result("semantic_validation_requirements_preserved", True),
        _gate_result("forbidden_rival_inventory_preserved", True),
        _gate_result("authorization_consumed", False, ["authorization remains unconsumed"], record=False),
        _gate_result("candidate_generated", False, ["candidate has not been generated"], record=False),
        _gate_result("no_model_calls", True),
        _gate_result(
            "project_health_scope_guard_passed",
            health["passed"] is True,
        ),
        _gate_result("no_final_claim", True),
        _gate_result("no_phase_shift_claim", True),
        _gate_result(
            "finalization_eligible",
            False,
            ["generation authorization is not finalization evidence"],
            record=False,
        ),
    ]
    failed_gates = [
        str(gate["gate_name"]) for gate in gate_results if not bool(gate["passed"])
    ]
    blockers = [
        "authorization remains unconsumed",
        "candidate has not been generated",
        "ablation has not been executed",
        "reader-state evaluation has not been run",
        "synthesis has not been run",
        "strongest rival remains blocking",
        "finalization remains refused",
    ]
    return {
        "accepted": True,
        "passed": False,
        "eligible": False,
        "source_work_order_packet_id": subject.work_order_packet_id,
        "decision": decision,
        "generation_authorized": generation_authorized,
        "next_generation_authorized": generation_authorized,
        "generation_attempt_budget": 1 if generation_authorized else 0,
        "authorization_consumed": False,
        "candidate_generated": False,
        "model_calls": 0,
        "finalization_eligible": False,
        "not_finalization_eligible": True,
        "no_final_claim": True,
        "no_phase_shift_claim": True,
        "strongest_rival_defeated": False,
        "strongest_rival_pressure_remains_blocking": True,
        "gate_results": gate_results,
        "failed_gates": failed_gates,
        "missing_gates": [],
        "final_gates_marked_passed": [],
        "unresolved_blockers": blockers,
        "summary_verdict": (
            "One bounded future nonlocal law-guided generation attempt is "
            "authorized but unconsumed; no candidate evidence exists yet."
        )
        if generation_authorized
        else "Operator review recorded a non-authorizing generation decision.",
        "worker": "nonlocal_law_generation_authorization_gate_report_v1_controller",
    }


def _build_packet_summary(
    *,
    subject: NonlocalLawGenerationAuthorizationSubject,
    packet_dir: Path,
    decision: str,
    payloads: dict[str, dict[str, object]],
    artifacts: dict[str, ArtifactRecord],
) -> dict[str, object]:
    source = subject.payloads["nonlocal_law_guided_work_order_packet"]
    validation = payloads["materiality_semantic_validation_readiness_report"]
    protected = payloads["protected_strengths_and_forbidden_imitation_review"]
    evidence = payloads["post_generation_evidence_plan"]
    counts = packet_artifact_count_summary(
        required_artifact_types=NONLOCAL_LAW_GENERATION_AUTHORIZATION_ARTIFACT_TYPES,
        produced_artifact_types=list(artifacts),
        packet_artifact_type="nonlocal_law_guided_generation_authorization_packet",
    )
    generation_authorized = decision == AUTHORIZATION_DECISION_AUTHORIZE_ONE
    return {
        "accepted": True,
        "run_id": subject.run_id,
        "packet_id": packet_dir.name,
        "packet_dir": str(packet_dir),
        "source_work_order_packet_id": subject.work_order_packet_id,
        "source_work_order_packet_dir": str(subject.work_order_packet_dir),
        "source_strategy_packet_id": source.get("source_strategy_packet_id"),
        "source_diagnostic_packet_id": source.get("source_diagnostic_packet_id"),
        "current_best_candidate_packet_id": source.get("current_best_candidate_packet_id"),
        "proof_packet_id": source.get("proof_packet_id"),
        "reader_state_packet_id": source.get("reader_state_packet_id"),
        "law_id": source.get("law_id"),
        "selected_strategy_class": source.get("selected_strategy_class"),
        "work_order_kind": source.get("work_order_kind"),
        "target_scope": source.get("target_scope"),
        "intervention_scope": source.get("intervention_scope"),
        "affected_regions": list(source["affected_regions"]),
        "protected_regions": list(source["protected_regions"]),
        "decision": decision,
        "generation_authorized": generation_authorized,
        "next_generation_authorized": generation_authorized,
        "generation_attempt_budget": 1 if generation_authorized else 0,
        "authorization_consumed": False,
        "candidate_generated": False,
        "model_calls": 0,
        "materiality_requirements": list(validation["materiality_requirements"]),
        "semantic_validation_requirements": list(
            validation["semantic_validation_requirements"]
        ),
        "forbidden_rival_objects_or_sequence": list(
            protected["forbidden_rival_objects_or_sequence"]
        ),
        "forbidden_rival_imitation_modes": list(
            protected["forbidden_rival_imitation_modes"]
        ),
        "forbidden_regressions": list(protected["forbidden_regressions"]),
        "target_unit_ids": list(source["target_unit_ids"]),
        "ablation_controls": list(evidence["ablation_controls"]),
        "reader_state_focus": list(evidence["reader_state_focus"]),
        "strongest_rival_defeated": False,
        "strongest_rival_pressure_remains_blocking": True,
        "next_recommended_action": _next_action(decision),
        "recommended_next_action": _next_action(decision),
        "counts": {
            **counts,
            "model_calls": 0,
            "candidate_artifacts_created": 0,
            "authorization_artifacts": counts["produced_artifacts"],
        },
        "artifact_ids": {
            artifact_type: artifact.id for artifact_type, artifact in artifacts.items()
        },
        "artifact_types": [
            *artifacts,
            "nonlocal_law_guided_generation_authorization_packet",
        ],
        "gate_report": payloads["nonlocal_law_generation_authorization_gate_report"],
        "finalization_eligible": False,
        "not_finalization_eligible": True,
        "no_final_claim": True,
        "no_phase_shift_claim": True,
        "worker": "nonlocal_law_guided_generation_authorization_packet_v1_controller",
    }


def _result_payload(
    *,
    packet_dir: Path,
    artifacts: dict[str, ArtifactRecord],
    payloads: dict[str, dict[str, object]],
) -> dict[str, object]:
    packet = payloads["nonlocal_law_guided_generation_authorization_packet"]
    return {
        **packet,
        "artifact_paths": {
            artifact_type: str(packet_dir / f"{artifact_type}.json")
            for artifact_type in artifacts
        },
    }


def _newer_corrected_work_order_packet_id(
    *,
    config: AbiConfig,
    run_id: str,
    work_order_packet_dir: Path,
    source_strategy_packet_id: str,
) -> str | None:
    current_number = _packet_number(work_order_packet_dir.name)
    root = config.run_dir(run_id) / "nonlocal_law_guided_work_order"
    if current_number is None or not root.exists():
        return None
    for packet_dir in sorted(root.glob("packet_*"), reverse=True):
        packet_number = _packet_number(packet_dir.name)
        if packet_number is None or packet_number <= current_number:
            continue
        packet_path = packet_dir / "nonlocal_law_guided_work_order_packet.json"
        if not packet_path.exists():
            continue
        try:
            payload = _read_envelope_payload(packet_path)
        except (OSError, ValueError, TypeError):
            continue
        if (
            payload.get("accepted") is True
            and payload.get("source_strategy_packet_id") == source_strategy_packet_id
            and _work_order_has_authorization_surface(payload)
        ):
            return str(payload.get("packet_id") or packet_dir.name)
    return None


def _work_order_has_authorization_surface(payload: dict[str, Any]) -> bool:
    return (
        payload.get("ready_for_generation_authorization_review") is True
        and payload.get("future_generation_authorized") is False
        and payload.get("generation_authorized") is False
        and bool(_string_list(payload.get("materiality_requirements")))
        and bool(_string_list(payload.get("semantic_validation_requirements")))
    )


def _unconsumed_authorization_for_work_order(
    connection: sqlite3.Connection,
    subject: NonlocalLawGenerationAuthorizationSubject,
) -> ArtifactRecord | None:
    for artifact in list_artifacts(connection, subject.run_id):
        if artifact.type != "nonlocal_law_guided_generation_authorization_packet":
            continue
        payload = _artifact_payload(artifact)
        if (
            payload.get("accepted") is True
            and payload.get("source_work_order_packet_id")
            == subject.work_order_packet_id
            and payload.get("authorization_consumed") is False
        ):
            return artifact
    return None


def _artifact_payload(artifact: ArtifactRecord) -> dict[str, Any]:
    try:
        envelope = read_json_file(artifact.path)
    except (OSError, ValueError, TypeError):
        return {}
    if not isinstance(envelope, dict) or not isinstance(envelope.get("payload"), dict):
        return {}
    return envelope["payload"]


def _artifact_ids_from_packet(packet_payload: dict[str, Any]) -> dict[str, str]:
    raw = packet_payload.get("artifact_ids")
    if not isinstance(raw, dict):
        return {}
    return {
        str(artifact_type): str(artifact_id)
        for artifact_type, artifact_id in raw.items()
        if isinstance(artifact_type, str)
        and isinstance(artifact_id, str)
        and artifact_id
    }


def _artifact_for_path(
    connection: sqlite3.Connection,
    path: Path,
) -> ArtifactRecord | None:
    row = connection.execute(
        """
        SELECT *
        FROM artifacts
        WHERE path = ?
        ORDER BY created_at DESC, id DESC
        LIMIT 1
        """,
        (str(path),),
    ).fetchone()
    return row_to_artifact(row) if row is not None else None


def _packet_number(packet_id: str) -> int | None:
    suffix = packet_id.removeprefix("packet_")
    return int(suffix) if suffix.isdecimal() else None


def _next_action(decision: str) -> str:
    if decision == AUTHORIZATION_DECISION_AUTHORIZE_ONE:
        return NEXT_ACTION_AUTHORIZE
    if decision == AUTHORIZATION_DECISION_REQUIRE_REVISION:
        return NEXT_ACTION_REQUIRE_REVISION
    return NEXT_ACTION_PAUSE


def _placeholder_generation_payload(
    *,
    authorization_packet: Path,
    client_name: str,
    message: str,
) -> dict[str, object]:
    return {
        "accepted": False,
        "refused": True,
        "message": message,
        "client": client_name,
        "authorization_packet": str(authorization_packet),
        "generation_authorized": True,
        "next_generation_authorized": True,
        "authorization_consumed": False,
        "candidate_generated": False,
        "model_calls": 0,
        "finalization_eligible": False,
        "no_final_claim": True,
        "no_phase_shift_claim": True,
        "next_recommended_action": NEXT_ACTION_GENERATOR_NOT_IMPLEMENTED,
    }


def _resolve_path(config: AbiConfig, value: Path | str) -> Path:
    path = Path(value)
    if path.is_absolute():
        return path
    return config.root / path


def _refusal(
    *,
    work_order_packet: Path,
    decision: str | None,
    message: str,
) -> NonlocalLawGenerationAuthorizationResult:
    return NonlocalLawGenerationAuthorizationResult(
        exit_code=1,
        payload={
            "accepted": False,
            "refused": True,
            "message": message,
            "work_order_packet": str(work_order_packet),
            "decision": decision,
            "generation_authorized": False,
            "next_generation_authorized": False,
            "generation_attempt_budget": 0,
            "authorization_consumed": False,
            "candidate_generated": False,
            "model_calls": 0,
            "finalization_eligible": False,
            "no_final_claim": True,
            "no_phase_shift_claim": True,
        },
    )


def _require_equal(
    payload: dict[str, Any],
    field_name: str,
    expected: object,
) -> None:
    if payload.get(field_name) != expected:
        raise ValueError(
            "Nonlocal law-guided generation authorization refused; "
            f"{field_name} must be {expected}."
        )


def _require_bool(
    payload: dict[str, Any],
    field_name: str,
    expected: bool,
) -> None:
    if payload.get(field_name) is not expected:
        raise ValueError(
            "Nonlocal law-guided generation authorization refused; "
            f"{field_name} must be {str(expected).lower()}."
        )


def _required_string(value: object, message: str) -> str:
    if isinstance(value, str) and value:
        return value
    raise ValueError(message)


def _string_list(value: object) -> list[str]:
    if not isinstance(value, list):
        return []
    return [str(item) for item in value if item]


def _has_final_or_phase_claim(payloads: dict[str, dict[str, Any]]) -> bool:
    return any(_payload_has_final_or_phase_claim(payload) for payload in payloads.values())


def _payload_has_final_or_phase_claim(value: object) -> bool:
    if isinstance(value, dict):
        for key, item in value.items():
            if key in {"finalization_eligible", "final_artifact", "final_claim"}:
                if item is True:
                    return True
            if key in {
                "phase_shift_claim",
                "strongest_rival_defeated",
                "strongest_rival_defeat_claim",
            }:
                if item is True:
                    return True
            if key in {"no_final_claim", "no_phase_shift_claim"}:
                if item is False:
                    return True
            if _payload_has_final_or_phase_claim(item):
                return True
    elif isinstance(value, list):
        return any(_payload_has_final_or_phase_claim(item) for item in value)
    return False


def _check(
    check_id: str,
    passed: bool,
    blockers: list[str] | None = None,
) -> dict[str, object]:
    return {
        "check_id": check_id,
        "passed": passed,
        "blocking_defects": blockers or ([] if passed else [f"{check_id} failed"]),
    }


def _gate_result(
    gate_name: str,
    passed: bool,
    blockers: list[str] | None = None,
    *,
    record: bool = True,
) -> dict[str, object]:
    return {
        "gate_name": gate_name,
        "passed": passed,
        "blocking_defects": blockers or ([] if passed else [f"{gate_name} failed"]),
        "recorded_as_passed_gate": record and passed,
    }


def _unique(values: list[str | None]) -> list[str]:
    seen: set[str] = set()
    output: list[str] = []
    for value in values:
        if not value or value in seen:
            continue
        seen.add(value)
        output.append(value)
    return output
