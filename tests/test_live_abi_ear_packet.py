...
from abi.model_schemas import ABI_EAR_VARIANTS_SCHEMA
from abi.modules.live_abi_ear import (
    LIVE_ABI_EAR_PACKET_ARTIFACT_TYPES,
    LIVE_ABI_EAR_PACKET_LINEAGE_ID,
...
def test_fake_live_packet_creates_required_artifacts_and_model_calls(tmp_path):
    config = config_for(tmp_path)

    result = run_live_abi_ear_packet_demo(config, client_name="fake")

    assert result.exit_code == 0
    assert result.payload["accepted"] is True
    assert result.payload["client"] == "fake"
    assert result.payload["counts"]["variants"] == 10
    assert result.payload["counts"]["moves"] == 20
    assert result.payload["counts"]["prose_inventions"] == 3
    assert set(result.payload["artifact_ids"]) == set(LIVE_ABI_EAR_PACKET_ARTIFACT_TYPES)
    ...
    model_artifact_types = {schema.artifact_type for schema in LIVE_ABI_EAR_PACKET_MODEL_SCHEMAS}
    with connect(config.db_path) as connection:
        artifacts = list_artifacts(connection, result.payload["run_id"])
        latest_run = get_latest_run(connection)

    assert latest_run.active_phase == "phase8_live_abi_ear_packet"
    ...
    assert artifact_by_type["live_abi_ear_packet"].lineage_id == LIVE_ABI_EAR_PACKET_LINEAGE_ID
    assert artifact_by_type["live_abi_ear_gate_report"].parent_ids


def test_fake_live_packet_is_deterministic_and_uses_unique_packet_dirs(tmp_path):
    config = config_for(tmp_path)

    first = run_live_abi_ear_packet_demo(config, client_name="fake")
    second = run_live_abi_ear_packet_demo(config, client_name="fake")
...
def test_fake_live_packet_budget_guard_refuses_before_run(tmp_path):
    config = config_for(tmp_path)
...
def test_fake_live_packet_invalid_output_records_validation_failure_without_artifact(tmp_path):
    config = config_for(tmp_path)
    ...
    result = run_live_abi_ear_packet_demo(
        ...
        client_name="fake",
        fake_mode="invalid",
        fake_target_schema=ABI_EAR_VARIANTS_SCHEMA,
    )

    assert result.exit_code == 1
    failed_call = result.payload["model_calls"][-1]
    assert failed_call["schema_name"] == ABI_EAR_VARIANTS_SCHEMA.name
    assert failed_call["status"] == MODEL_CALL_VALIDATION_FAILED
    assert failed_call["parsed_output_artifact_id"] is None
    assert ABI_EAR_VARIANTS_SCHEMA.artifact_type not in result.payload["artifact_ids"]

    with connect(config.db_path) as connection:
        artifacts = list_artifacts(connection, result.payload["run_id"])

    assert {artifact.type for artifact in artifacts} == {
        "model_abi_ear_germ_analysis",
        "model_abi_ear_field_model",
    }


def test_fake_live_packet_client_failure_records_failure_without_artifact(tmp_path):
    config = config_for(tmp_path)
    ...
    assert failed_call["parsed_output_artifact_id"] is None
    assert "simulated fake packet client failure" in failed_call["error_message"]
...
def test_openai_live_packet_refuses_without_allow_before_run(tmp_path, monkeypatch):
    monkeypatch.setenv("OPENAI_API_KEY", "stub-key")
    config = config_for(tmp_path)

    result = run_live_abi_ear_packet_demo(config, client_name="openai")
...
def test_openai_live_packet_refuses_without_openai_key_before_run(tmp_path, monkeypatch):
...
def test_live_packet_cli_fake_and_openai_guard(tmp_path, capsys, monkeypatch):
    monkeypatch.delenv("OPENAI_API_KEY", raising=False)

    fake_exit = main(["--root", str(tmp_path), "ear", "live-demo", "--client", "fake"])
    fake_payload = json.loads(capsys.readouterr().out)

    assert fake_exit == 0
    assert fake_payload["client"] == "fake"
    assert fake_payload["counts"]["variants"] == 10

    openai_exit = main(["--root", str(tmp_path), "ear", "live-demo", "--client", "openai"])
...
def test_fake_live_packet_preserves_finalization_fail_closed(tmp_path):
    config = config_for(tmp_path)
    result = run_live_abi_ear_packet_demo(config, client_name="fake")
